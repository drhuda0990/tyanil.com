@auth('customer')
    @if (
        $service->customerHasPurchased(\Auth::guard('customer')->user()->id) &&
            !$service->customerHasRated(\Auth::guard('customer')->user()->id))
        <div class="container">
            <hr>
            <div class="feedback">
                <form method="POST" class="form-box rtlDirection w-100" action="{{ route('rating') }}">
                    @csrf
                    <div class="rating">
                        <!-- Hidden service id -->
                    ...
                    <div class="mt-3">
                        <textarea name="comment" class="form-control" rows="3" placeholder="أكتب تقييمك هنا"></textarea>
                        @error('comment')
                    ...
                    <div class="row justify-content-left d-flex">
                        <div class="col-md-4 d-flex flex-column">
                            <div class="rating-box">
                                <h1 class="pt-4">{{ $avg }}</h1>
                                <p class="">من 5</p>
                            </div>
                            <div>
                                @for ($i = 1; $i <= 5; $i++)
                                    <span
                                        class="fa fa-star {{ $i <= round($avg) ? 'star-active' : 'star-inactive' }} mx-1"></span>
                                @endfor
                        ...
                        <div class="col-md-8">
                            <div class="rating-bar0 justify-content-center">
                                <table class="text-left mx-auto">
                                    <tr>
                                        <td class="rating-label"><svg class="rating-5"
                                        ...
                                        <td class="rating-bar">
                                            <div class="bar-container">
                                                @php
                                                    $percent5 = $total ? ($stars[5] / $total) * 100 : 0;
                                                @endphp

                                                <div class="bar-5" style="width: {{ $percent5 }}%"></div>
                                            </div>
                                        </td>
                                        <td class="text-right">{{ $stars[5] }}</td>
                                    </tr>
                                    <tr>
                                        <td class="rating-label"> <svg class="rating-4"
                                                xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
                                                <circle cx="256" cy="256" r="256" fill="#ffd93b" />
                                                <path
                                                    d="M512 256A256 256 0 0 1 56.7 416.7a256 256 0 0 0 360-360c58.1 47 95.3 118.8 95.3 199.3z"
                                        ...
                                        <td class="rating-bar">
                                            <div class="bar-container">
                                                @php
                                                    $percent5 = $total ? ($stars[4] / $total) * 100 : 0;
                                                @endphp

                                                <div class="bar-4" style="width: {{ $percent5 }}%"></div>
                                            </div>
                                        </td>
                                        <td class="text-right">{{ $stars[4] }}</td>
                                    </tr>
                                    <tr>
                                        ...
                                        <td class="rating-bar">
                                            <div class="bar-container">
                                                @php
                                                    $percent5 = $total ? ($stars[3] / $total) * 100 : 0;
                                                @endphp

                                                <div class="bar-3" style="width: {{ $percent5 }}%"></div>
                                            </div>
                                        </td>
                                        <td class="text-right">{{ $stars[3] }}</td>
                                    </tr>
                                    <tr>
                                        <td class="rating-label">
                                        ...
                                        <td class="rating-bar">
                                            <div class="bar-container">
                                                @php
                                                    $percent5 = $total ? ($stars[2] / $total) * 100 : 0;
                                                @endphp

                                                <div class="bar-2" style="width: {{ $percent5 }}%"></div>
                                            </div>
                                        </td>
                                        <td class="text-right">{{ $stars[2] }}</td>
                                    </tr>
                                    <tr>
                                        <td class="rating-label">
                                        ...
                                        <td class="rating-bar">
                                            <div class="bar-container">
                                                @php
                                                    $percent5 = $total ? ($stars[1] / $total) * 100 : 0;
                                                @endphp

                                                <div class="bar-1" style="width: {{ $percent5 }}%"></div>
                                            </div>
                                        </td>
                                        <td class="text-right">{{ $stars[1] }}</td>
                                    </tr>
                                </table>
                            </div>
                        </div>
                    ...
                    <div class="card ratingCard mb-3">
                        <div class="row d-flex align-items-center justify-content-center ">
                            ...
                            <div class="d-flex flex-column">
                                <h5 class="mt-2 mb-0">{{ $rating->customer->name }}</h5>
                                <div>
                                    @for ($i = 1; $i <= 5; $i++)
                                        <span
                                            class="fa fa-star {{ $i <= $rating->stars ? 'star-active' : 'star-inactive' }}"></span>
                                    @endfor
                                </div>
                            </div>
                            <div class="ml-auto">
                                <p class="text-muted">
                                    {{ $rating->created_at->format(' M Y') }}
                                </p>
                            </div>
                        ...
                        @if ($rating->comment)
                            <div class="row text-left">
                                <p class="content mt-2">{{ $rating->comment }}</p>
                            </div>
                        @endif
                    </div>
                @endforeach
